...
export async function createUserAction(prevState: ActionState, formData: FormData): Promise<ActionState> {
    ...
    return { success: true, redirectUrl: `/dashboard/users/${newUserId}`, message: "Utente creato con successo." };
}
...
export async function updateUserAction(prevState: ActionState, formData: FormData): Promise<ActionState> {
    ...
    try {
        ...
        const updated = await userService.updateUser(userData, "PATCH");
    ...
    revalidatePath("/dashboard/users");
    revalidateTag('users-list-tag', 'default');
    revalidateTag(`user-${updatedUserId}`, 'default');
    return { success: true, redirectUrl: `/dashboard/users/${updatedUserId}`, message: "Utente modificato con successo." };
}
