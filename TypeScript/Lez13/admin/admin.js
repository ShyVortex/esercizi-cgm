...
async function fetchData() {
    ...
    try {
        ...
        if (!query) {
            ...
            const totalCount = Math.ceil(Number(headers.get('X-Total-Count')) / state.per_page) || 1;
...
btnPrev.addEventListener('click', () => { if (state.page > 1) {
    state.page--;
    fetchData();
} });
...
window.toggleBin = toggleBin;
window.logout = logout;
window.openCreateModal = openCreateModal;
window.fetchData = fetchData;
window.physicalDelete = physicalDelete;
window.restoreItem = restoreItem;
window.editItem = editItem;
window.logicalDelete = logicalDelete;
